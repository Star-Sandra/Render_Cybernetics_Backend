...
from smartmama.schemas.auth_schema import UserSignup, UserLogin, TokenResponse
...
def deactivate_chv(chv_id: UUID, reason: str, db: Session = Depends(get_db),
                    current_supervisor: TokenPayload = Depends(require_supervisor)):
    updated = chv_service.deactivate_chv_by_supervisor(db, chv_id, current_supervisor.user_id, reason)
    return {
...
def approve_cert(chv_id: UUID, db: Session = Depends(get_db),
                  current_supervisor: TokenPayload = Depends(require_supervisor)):
    updated = chv_service.approve_certificate(db, chv_id, current_supervisor.user_id)
    return {
        "chv_id": updated.chv_id,
        "user_id": updated.user_id,
        "first_name": updated.user.person.first_name,
...
def delete_chv_profile(
    ...
    chv_service.delete_own_profile(db, current_chv.user_id)
    return
...
def reject_cert(chv_id: UUID, notes: str, db: Session = Depends(get_db),
                 current_supervisor: TokenPayload = Depends(require_supervisor)):
    updated = chv_service.reject_certificate(db, chv_id, current_supervisor.user_id, notes)
    return {
        "chv_id": updated.chv_id,
        "user_id": updated.user_id,
        "first_name": updated.user.person.first_name,
